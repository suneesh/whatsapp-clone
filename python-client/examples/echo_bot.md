...
| `--user` | `echobot` | Bot username |
| `--password` | `bot_password` | Bot password |
...
1. **Initialization**: The bot parses command-line arguments for server URL and credentials
2. **Authentication**: Attempts to register a new user, falls back to login if already registered
3. **Event Handler**: Uses the `@client.on_message` decorator to register a message handler
4. **Echo Logic**: When a message is received, it sends back the same content prefixed with "Echo: "
5. **Keep Alive**: Uses `asyncio.Event().wait()` to keep the bot running indefinitely
